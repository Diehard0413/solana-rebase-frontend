...
export interface RTCFireSessionOptions {
  ...
  /**
   * An Boolean representing the current user if he can stream. Could come from Firebase Auth or elsewhere.
   */
  isStreamer: boolean;
...
class RTCFireSession {
  ...
  constructor(options: RTCFireSessionOptions) {
    ...
    (async () => {
      try {
        // if(this.options.isStreamer){
          DEFAULT_MEDIA_CONSTRAINTS.audio = true;
          DEFAULT_MEDIA_CONSTRAINTS.video = this.options.isStreamer;
  ...
  private onRemoveParticipant(pid) {
    ...
    const { onReadValueChange, readRef, writeRef, writeRefOnDisconnect } = this.participantInfo[pid];
